...
// Remove flipbook assignment and display meta from all products.
...
// Remove flipbook variation flag from all variations.
$wpdb->query( "DELETE FROM {$wpdb->postmeta} WHERE meta_key = '_flipbook'" );

// Remove legacy flipbook_id post meta (replaced by post_id-based lookup).
$wpdb->query( "DELETE FROM {$wpdb->postmeta} WHERE meta_key = 'flipbook_id'" );

// Remove purchased-flipbooks transients.
$wpdb->query(
